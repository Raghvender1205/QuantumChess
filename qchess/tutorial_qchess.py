...
class TutorialQChess(QChess):
    def __init__(self, tutorial_mode):
        ...
        self.move_types = [
            {'name': 'Standard', 'move_number': 2,
                'func': TutorialQChess.standard_move},
        ...
        for json_step in tutorial_mode['tutorial_steps']:
            ...
            possible_point_names = ['source', 'source1', 'source2', 'target',
                                    'target1', 'target2', 'forbidden_target', 'required_target']
            ...
            for point_name in possible_point_names:
                if point_name in json_valid_moves:
                    ...
                    if points:
                        ...
                        raise ValueError(
                            'Invalid {} squares'.format(point_name))
    ...
    def ascii_main_loop(self):
        self.combined_message += '{}.- {}\n\n'.format(
            self.step_index + 1, self.initial_message)
    ...
    def split_move(self, source, target1, target2):
        ...
        if 'valid_moves' in step:
            ...
            #check at least one of the targets is in required_target
            if 'required_target' in valid_moves:
                move = valid_moves['required_target']

                if not target1 in move and not target2 in move:
                    return False

            #check targets are not forbidden
            if 'forbidden_target' in valid_moves:
                move = valid_moves['forbidden_target']
