...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Quantum Chess.', epilog=epilog)
    ...
    group.add_argument(
        '--tutorial', help='run a specific tutorial from its configuration file in tutorials/', metavar='FILE')

    group.add_argument('--guided-tutorials',
                       help='run all the tutorials in order and keep track of the progress made', action='store_true')
    ...
    if args.guided_tutorials:
        ...
        if args.tutorial:
            ...
            try:
                json_data = open(os.path.join(
                    'game_modes', args.game_mode + '.json'))
            except FileNotFoundError:
